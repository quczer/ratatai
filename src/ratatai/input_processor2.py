...
class OpenAiInputProcessor(InputProcessor):
    def __init__(self, recipe_steps: list[str], openai_api_key: str):
        ...
        def __finish_cooking(_: int) -> str:
            self.finished = True
            print("Congratulations! You've completed the recipe. Enjoy your meal!")
    ...
    def generate_response(self, text_prompt: str) -> str:
        ...
        print('self.completed_steps: ', self.completed_steps)
        ...
        # Check for the step with the lowest number that hasn't been completed yet
        
        remaining_steps = [step for step in range(1, len(self.recipe_steps) + 1) if step not in self.completed_steps]

        if remaining_steps:
            # Set the current step to the lowest remaining step
            self.current_step = min(remaining_steps) - 1  # Convert to 0-indexed
        else:
            # If no steps remain, mark finished automatically.
            self.finished = True
            self.memory.chat_memory.add_message(
